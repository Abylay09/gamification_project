...
function BuyModal({ show, close, cost, uid, showSuccess, myMoney }) {
    ...
    return (
        <Modal dialogClassName='custom-modal' show={show} onHide={close}>
            ...
            <Modal.Footer bsPrefix="custom-footer">
                {myMoney < quantity * cost ? <span className='text-center mb-2' style={{color : "red"}}>Недостаточно средств</span> : ""}
                <Button onClick={() => {
                    if (myMoney < quantity * cost) {
                        return
                    } else {
                        buy()
                    }
                }} className='w-100 py-3 mt-auto ' style={{ marginBottom: "64px", }} >
